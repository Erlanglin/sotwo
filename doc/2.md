...
目前sotwo基础系统主要由sotwo数据库、sotwo-core模块、sotwo-db模块、
sotwo-os-api模块和sotwo-all模块组成。

实际上，属于sotwo**真正的基础系统**是sotwo-core模块和sotwo-db模块。
sotwo-os-api模块只是为了减少开发中对第三方图片存储服务依赖而实现的简单图像存储服务，
建议开发者最终部署时切换到第三方图片存储服务。sotwo-all模块只是一个包裹模块，
没有任何代码，其作用是融合三个spring boot模块和sotwo-adminm模块静态文件到
一个单独spring boot应用中，并最终打包成war格式的项目安装包。

目前存在的问题：

* `严重`数据库采用git，每次跟新都是5MB数据，影响项目下载速度
* `改善` sotwo-db的一些CRUD操作可以基于开源库重构
* `功能`可以参考一些云存储服务的API加强一些功能

## 2.1 sotwo.sql

sotwo.sql数据库基于nideshop中的[nideshop.sql](https://github.com/tumobi/nideshop/blob/master/nideshop.sql)数据库，然后在实际开发过程中进行了调整和修改:

* 删除了一些目前不必要的表；
* 删除了表中一些目前不必要的字段；
* 行政区域数据sotwo_region没有采用原nideshop中的数据，而是采用了[Administrative-divisions-of-China](https://github.com/modood/Administrative-divisions-of-China)；
* 表中的某些字段采用JSON；
* 表中的日期或时间字段采用DATE、DATETIME；
* 字段的数据类型粗粒度化，例如避免MEDIUMINT，而是INT；
* 表的数据做了清理、调整和补充（假数据）。

具体不同可以比较sotwo-db模块下sql文件夹中nideshop.sql和sotwo.sql。
...
目前准备支持用户普通账号登录和微信登录两种方式，两种登录方式仅仅采用一个sotwo-user表可能不是很合适。此外，如果进一步支持其他多种第三方登录，那么这里需要重新设计。

### 2.1.3 行政区域设计
 
原nideship.sql中存在region数据，但是sotwo.sql的region数据则来自
[Administrative-divisions-of-China](https://github.com/modood/Administrative-divisions-of-China)项目。从该项目中导入数据到sotwo.sql的sotwo-province、sotwo-city、sotwo-area和sotwo-street四个表，然后重新生成一个新的sotwo-region表。
...
sotwo-db模块是一个普通的Spring Boot应用，基于mybatis技术提供开发者
访问数据库的功能。

此外，sotwo-db最终是作为一个类库被其他模块所依赖使用，因此并不对外
...
2. 自动生产src文件夹下domain包内的sotwoBrand.java类、sotwoBrandExample.java类、
    dao包内的sotwoBrandMapper.java接口和resources文件夹下dao包内的sotwoBrandMapper.xml文件。

3. 手动在service包内创建sotwoBrandService.java来对外提供具体的服务。
...
1. 在数据库里面创建一个表，例如`sotwo_demo`:

    ```sql
    CREATE TABLE `sotwo`.`sotwo_demo` (
      `id` INT NOT NULL AUTO_INCREMENT,
      `name` VARCHAR(45) NULL,
      `address` VARCHAR(45) NULL,
      PRIMARY KEY (`id`));
      
    INSERT INTO `sotwo`.`sotwo_demo` (`id`, `name`, `address`) 
...
2. 在generatorConfig.xml中增加一个新的table标签
    ...
    ```
    <generatorConfiguration>
         <table tableName="sotwo_demo">
             <generatedKey column="id" sqlStatement="MySql" identity="true" />
         </table>
    </generatorConfiguration>
    ```
3. 双击mybatis generator插件，检查sotwoDemo.java类、sotwoDemoExample.java类、
   sotwoDemoMapper.java接口和sotwoDemoMapper.xml是否生产。
   
4. 在service里面新建sotwoDemoService.java类，

   ```java
    @Service
    public class sotwoDemoService {
       @Resource
       private sotwoDemoMapper demoMapper;

        public List<sotwoDemo> list() {
           sotwoDemoExample example = new sotwoDemoExample();
           return demoMapper.selectByExample(example);    
        }
    }
   ```

5. 可以在`src/test/java/org.myoranges.sotwo.db`包里面创建sotwoDemoTest.java类,
    使用Junit进行测试。
    ...
    @SpringBootTest
    public class sotwoDemoTest {    
       @Autowired
       private sotwoDemoService demoService;
    
       @Test
       public void test() {    
        List<sotwoDemo> sotwoDemoList = demoService.list();
        Assert.assertTrue(sotwoDemoList.size() != 0);
       }
    
    }
    ```

6. 同样地，可以在Controller中使用sotwoDemoService来对外提供服务。
    ...
    public class DemoController {
       ...
       @RequestMapping("/list")
       public Object list(){    
           List<sotwoDemo> demoList = demoService.list();   
           return demoList;
       }
    }
    ```
...
sotwo-core模块是spring boot应用通用的代码，包括配置代码和util代码。
...
注意
> 这里的util代码不会涉及具体业务，例如sotwo-db模块中存在一个
> OrderUtil类处理数据库中sotwo_order表的一些转换工作。
...
作为后台模块之一，sotwo-os-api并没有对应的前端模块，而只是在sotwo-admin模块
的对象存储页面中允许管理员修改。
...
查看sotwo-all模块，代码仅仅只有一个Application类。

实际的原理是sotwo-all模块内的pom.xml文件：

1. 声明打包方式是`war`，因此最后会打包war格式

2. 设置`spring-boot-starter-tomcat`包是`provided`，因此最终不会打包

3. 申明依赖`sotwo-os-api`、`sotwo-wx-api`和`sotwo-admin-api`，
   因此最终会打包；
...
4. 利用maven-resources-plugin插件，将sotwo-admin模块下编译得到dist文件下的
   静态文件打包到static目录中。
   
   注意，这里只是简单的复制。因此开发者需要保证sotwo-all打包前，sotwo-admin
   模块内dist目录下静态文件已经生成。
